...
function deriveNormalizedBaseName(storedCanonicalField: string): string {
  ...
  return storedCanonicalField.split('|')[0]?.trim() ?? storedCanonicalField.trim();
}

function hasStructuredConflicts(products: ProductWithAliases[]): boolean {
  const strengths = new Set(products.map((product) => product.strength).filter(Boolean));
  const formulations = new Set(products.map((product) => product.dosageForm).filter(Boolean));
  const packSizes = new Set(products.map((product) => product.packSize).filter(Boolean));
...
function buildAliasCollisionMap(products: ProductWithAliases[]): Map<string, Set<string>> {
  ...
  for (const product of products) {
    for (const alias of product.aliases) {
      ...
      const existing = aliasCollisionMap.get(canonicalAlias) ?? new Set<string>();
  ...
  return {
    async listLikelyDuplicateGroups(): Promise<ProductDuplicateGroup[]> {
      ...
      for (const [groupKey, groupedProducts] of structuredGroups.entries()) {
        if (groupedProducts.length < 2 || hasStructuredConflicts(groupedProducts)) {
          continue;
        }
        ...
        groups.push({
          ...
          confidence:
            groupedProducts.every((product) => product.strength || product.dosageForm || product.packSize)
              ? 'HIGH'
              : 'MEDIUM',
          products: mapProductsForOutput(groupedProducts),
        });
        seenGroupKeys.add(groupedProducts.map((product) => product.id).sort().join('|'));
      }
      ...
      for (const [canonicalAlias, productIds] of aliasCollisionMap.entries()) {
        ...
        const groupedProducts = products.filter((product) => productIds.has(product.id));
        const stableKey = groupedProducts.map((product) => product.id).sort().join('|');

        if (seenGroupKeys.has(stableKey) || hasStructuredConflicts(groupedProducts)) {
          continue;
        }
        ...
        groups.push({
          ...
          confidence:
            groupedProducts.every(
              (product) => product.strength || product.dosageForm || product.packSize,
            )
              ? 'HIGH'
              : 'MEDIUM',
          products: mapProductsForOutput(groupedProducts),
