...
test('deterministic high-confidence email body does not call AI fallback', async () => {
  ...
  const rawBody = [
    ...
    'Paracetamol 500mg caplets 16 : Ã‚Â£1.25',
  ].join('\r\n');
  ...
  const result = await parseStructuredPriceText(rawBody, {
    aiOfferParser: {
      parseText: async () => {
        called = true;
        throw new Error(
          'AI fallback should not run for high-confidence deterministic parsing.',
        );
      },
    },
    source: 'EMAIL_BODY',
...
test('messy but commercially relevant email body can use AI fallback and return structured output', async () => {
  const bodyText =
    'Acme can do Paracetamol 500mg caplets 16 at 1.25 GBP if needed, MOQ 20.';
          ...
          offers: [
            {
              ...
              confidence: 'MEDIUM',
              reason:
                'Message states a product, price, currency, and MOQ in one sentence.',
            },
          ],
  ...
  assert.equal(
    result.parsedRows[0]?.rawProductText,
...
test('unusable AI output falls back safely without crashing', async () => {
  const result = await parseStructuredPriceText(
    'Please send your best offer around 8.40.',
    {
      aiOfferParser: {
        parseText: async () => ({
          status: 'unusable',
  ...
  assert.match(
    result.aiFallbackRejectedReason ?? '',
    /unusable structured data/i,
  );
...
test('AI fallback result remains review-oriented when confidence is medium or low', async () => {
  const result = await parseStructuredPriceText(
    'Stock maybe available for Metformin around 3.10 GBP.',
    {
      aiOfferParser: {
        parseText: async () => ({
          status: 'success',
            ...
            offers: [
              {
                ...
                confidence: 'LOW',
                reason:
                  'The message is ambiguous and does not clearly specify the pack or formulation.',
              },
            ],
