...
const approvalStatusSchema = z.enum(['PENDING_APPROVAL', 'APPROVED', 'REJECTED', 'CANCELLED']);
...
const markOrderStatusBodySchema = z.object({
...
const updateReferenceBodySchema = z.object({
...
buyDecisionsRouter.get('/', asyncHandler(async (request, response) => {
  const { query } = parseRequest<unknown, z.infer<typeof listBuyDecisionsQuerySchema>>(request, {
...
buyDecisionsRouter.get('/:id', asyncHandler(async (request, response) => {
  const { params } = parseRequest<z.infer<typeof idParamSchema>>(request, {
    params: idParamSchema,
  });

  response.json({
    item: requireFound(await buyDecisionService.getBuyDecision(params.id), 'Buy decision not found.'),
  });
}));

buyDecisionsRouter.patch('/:id', requireInternalOperatorAccess, asyncHandler(async (request, response) => {
