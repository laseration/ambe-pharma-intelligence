...
import type { EmailInboundMessage, EmailInboundResult } from '../email/inbound/types';
import { db } from '../lib/db';
import { listReviewQueueItems, type ReviewQueueItem } from '../reviewQueue/service';

export const LOCAL_ACCOUNT_OPENING_TEST_SENDER = 'local-account-opening@example.test';
export const LOCAL_ACCOUNT_OPENING_REVIEW_URL = 'http://localhost:3000/dashboard/review';
...
type CreateAccountOpeningTestCaseDependencies = {
  ...
  findCaseByFingerprint: (sourceFingerprint: string) => Promise<PersistedAccountOpeningCaseSummary | null>;
...
export async function createAccountOpeningTestCase(
  ...
  const offerWorkflowItemCountBefore = await dependencies.countOfferWorkflowItems();
  const result = await dependencies.ingestMessage(buildLocalAccountOpeningTestMessage());
  const accountOpeningCase = result.items[0]?.accountOpeningCase;

  if (!accountOpeningCase) {
    throw new Error('Local test message did not produce an account-opening case.');
  }

  const persistedCase = await dependencies.findCaseByFingerprint(accountOpeningCase.sourceFingerprint);
  ...
  const offerWorkflowItemCountAfter = await dependencies.countOfferWorkflowItems();
  ...
  return {
    ...
    offerWorkflowItemCountDelta: offerWorkflowItemCountAfter - offerWorkflowItemCountBefore,
  };
}
...
async function main() {
  ...
  console.log(`Review queue item found: ${result.reviewQueueItemFound ? 'yes' : 'no'}`);
  console.log(`BuyDecision count delta: ${result.buyDecisionCountDelta}`);
  console.log(`OfferWorkflowItem count delta: ${result.offerWorkflowItemCountDelta}`);
}

if (require.main === module) {
  main()
    .catch((error: unknown) => {
      console.error(error instanceof Error ? error.message : 'Failed to create account-opening test case.');
      process.exitCode = 1;
    })
    .finally(async () => {
      await db.$disconnect();
