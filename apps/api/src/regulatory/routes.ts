...
const regulatoryIngestBodySchema = z
  .object({
...
const reviewItemPatchBodySchema = z
  .object({
    status: regulatoryAlertStatusSchema.optional(),
    note: optionalTrimmedStringSchema,
...
regulatoryRouter.get(
  ...
  asyncHandler(async (request, response) => {
    ...
    response.json({
      item: requireFound(
        await getRegulatoryUpdate(params.id),
...
regulatoryRouter.post(
  ...
  asyncHandler(async (request, response) => {
    const { body } = parseRequest<
      unknown,
      unknown,
      z.infer<typeof regulatoryIngestBodySchema>
    >(request, {
      body: regulatoryIngestBodySchema,
    });

    response.status(201).json({
      item: await ingestRegulatoryUpdate(
        body,
...
regulatoryRouter.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    response.status(201).json({
      item: await parseStoredRegulatoryUpdate(
        params.id,
        resolveInternalActor(request, body),
      ),
    });
...
regulatoryRouter.get(
  ...
  asyncHandler(async (request, response) => {
    ...
    response.json({
      item: requireFound(
        await getRegulatorySignal(params.id),
...
regulatoryRouter.get(
  ...
  asyncHandler(async (request, response) => {
    const { query } = parseRequest<
      unknown,
      z.infer<typeof listByStatusQuerySchema>
    >(request, {
...
regulatoryRouter.get(
  ...
  asyncHandler(async (request, response) => {
    ...
    response.json({
      item: requireFound(
        await getRegulatoryAlert(params.id),
...
regulatoryRouter.get(
  ...
  asyncHandler(async (request, response) => {
    ...
    response.json({
      item: requireFound(
        await getRegulatoryReviewItem(params.id),
