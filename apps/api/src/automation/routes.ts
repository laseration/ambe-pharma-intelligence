...
const updateReadinessBodySchema = z.object({
  scopeName: optionalTrimmedStringSchema,
  globalMode: z
    .enum([
      'OBSERVE_ONLY',
...
automationRouter.get('/readiness', asyncHandler(async (request, response) => {
  const { query } = parseRequest<unknown, z.infer<typeof readinessQuerySchema>>(request, {
...
automationRouter.put('/readiness', requireInternalOperatorAccess, asyncHandler(async (request, response) => {
  const { body } = parseRequest<unknown, unknown, z.infer<typeof updateReadinessBodySchema>>(request, {
  ...
  response.json({
    item: await automationService.updateReadinessPolicy({
      ...
      minimumSupplierResolutionPrecisionPct: body.minimumSupplierResolutionPrecisionPct,
...
automationRouter.get('/evaluation', asyncHandler(async (request, response) => {
  const { query } = parseRequest<unknown, z.infer<typeof readinessQuerySchema>>(request, {
    query: readinessQuerySchema,
  });
...
automationRouter.post('/feedback', requireInternalOperatorAccess, asyncHandler(async (request, response) => {
  const { body } = parseRequest<unknown, unknown, z.infer<typeof operatorFeedbackSchema>>(request, {
