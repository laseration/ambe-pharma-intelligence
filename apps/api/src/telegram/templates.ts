...
export function buildOpportunityMessage(opportunity: OpportunityWithRelations): string {
  const metrics =
  ...
  const stock = typeof metrics?.currentStockQty === 'number' ? metrics.currentStockQty : null;
  ...
  if (opportunity.supplier?.name && ['BUY', 'PRICE_ALERT'].includes(opportunity.type)) {
  ...
  if (stock !== null && ['PUSH', 'RESTOCK', 'DEAD_STOCK'].includes(opportunity.type)) {
...
export function buildDailySummaryMessage(
  ...
  const grouped = opportunities.reduce<Record<string, number>>((counts, opportunity) => {
  ...
  const topOpportunity = [...opportunities].sort((left, right) => right.score - left.score)[0];
  if (topOpportunity) {
    lines.push('');
    lines.push('Top priority');
    lines.push(formatLine('Type', topOpportunity.type));
    lines.push(formatLine('Product', topOpportunity.product?.name ?? 'Unknown product'));
    lines.push(formatLine('Why', topOpportunity.description ?? 'Needs attention'));
