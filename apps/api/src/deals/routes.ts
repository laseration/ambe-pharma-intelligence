...
const tradeOpportunityStatusSchema = z.enum(['OPEN', 'ON_HOLD', 'DROPPED', 'WON', 'LOST']);
...
const tradeMessageDirectionSchema = z.enum(['TO_SUPPLIER', 'TO_BUYER', 'INTERNAL']);
...
const messagingPolicySchema = z.object({
...
const createDealBodySchema = z.object({
  sourceType: z.enum(['EMAIL_DERIVED_OFFER', 'WORKFLOW_ITEM', 'BUY_DECISION', 'OPERATOR_CREATED']).optional(),
  emailDerivedOfferId: optionalTrimmedStringSchema,
...
const updateDraftBodySchema = z.object({
  action: z.enum(['UPDATE', 'APPROVE', 'REJECT', 'CANCEL', 'MARK_SENT']).optional(),
  subject: optionalTrimmedStringSchema,
  body: optionalTrimmedStringSchema,
  note: optionalTrimmedStringSchema,
  metadata: z.unknown().optional(),
...
dealsRouter.get('/', asyncHandler(async (request, response) => {
  const { query } = parseRequest<unknown, z.infer<typeof listDealsQuerySchema>>(request, {
...
dealsRouter.get('/:id', asyncHandler(async (request, response) => {
...
dealsRouter.post('/', requireInternalOperatorAccess, asyncHandler(async (request, response) => {
  const { body } = parseRequest<unknown, unknown, z.infer<typeof createDealBodySchema>>(request, {
...
dealsRouter.patch('/:id', requireInternalOperatorAccess, asyncHandler(async (request, response) => {
...
dealsRouter.get('/:id/events', asyncHandler(async (request, response) => {
  const { params } = parseRequest<z.infer<typeof idParamSchema>>(request, {
    params: idParamSchema,
  });

  response.json({
    items: await tradeOpportunityService.listTradeOpportunityEvents(params.id),
  });
}));

dealsRouter.get('/:id/drafts', asyncHandler(async (request, response) => {
  const { params } = parseRequest<z.infer<typeof idParamSchema>>(request, {
    params: idParamSchema,
  });
...
dealsRouter.post('/:id/drafts', requireInternalOperatorAccess, asyncHandler(async (request, response) => {
  const { params, body } = parseRequest<
    z.infer<typeof idParamSchema>,
    unknown,
...
dealsRouter.patch('/drafts/:draftId', requireInternalOperatorAccess, asyncHandler(async (request, response) => {
  const { params, body } = parseRequest<
  ...
  response.json({
    item: await tradeOpportunityService.updateTradeMessageDraft(params.draftId, {
      ...body,
      ...resolveInternalActor(request, body),
    }),
  });
